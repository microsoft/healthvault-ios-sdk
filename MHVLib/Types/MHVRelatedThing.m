...
@implementation MHVRelatedThingCollection
...
- (instancetype)init
{
    ...
    if (self)
    {
        self.type = [MHVRelatedThing class];
    }
...
- (NSUInteger)indexOfRelation:(NSString *)name
{
    ...
    {
        MHVRelatedThing *thing = (MHVRelatedThing *)[self objectAtIndex:i];
        if (thing.relationship && [thing.relationship isEqualToStringCaseInsensitive:name])
        {
            return i;
        }
    }
