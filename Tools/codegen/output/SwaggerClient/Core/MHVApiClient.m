...
#import "MHVLogger.h"
#import "MHVApiClient.h"
#import "MHVJSONRequestSerializer.h"
#import "MHVQueryParamCollection.h"
#import "MHVDefaultConfiguration.h"

NSString *const MHVResponseObjectErrorKey = @"MHVResponseObject";

static NSString * const kMHVContentDispositionKey = @"Content-Disposition";

static NSDictionary * MHV__headerFieldsForResponse(NSURLResponse *response) {
...
static NSString * MHV__fileNameForResponse(NSURLResponse *response) {
    NSDictionary * headers = MHV__headerFieldsForResponse(response);
    if(!headers[kMHVContentDispositionKey]) {
    ...
    NSString *contentDispositionHeader = headers[kMHVContentDispositionKey];
...
@interface MHVApiClient ()

@property (nonatomic, strong, readwrite) id<MHVConfiguration> configuration;

@property (nonatomic, strong) NSArray<NSString*>* downloadTaskResponseTypes;

@end

@implementation MHVApiClient

#pragma mark - Singleton Methods

+ (instancetype) sharedClient {
    static MHVApiClient *sharedClient = nil;
...
- (instancetype)init {
    return [self initWithConfiguration:[MHVDefaultConfiguration sharedConfig]];
}

- (instancetype)initWithBaseURL:(NSURL *)url {
    return [self initWithBaseURL:url configuration:[MHVDefaultConfiguration sharedConfig]];
}
...
- (instancetype)initWithBaseURL:(NSURL *)url configuration:(id<MHVConfiguration>)configuration {
    ...
    if (self) {
        ...
        _timeoutInterval = 60;
        _responseDeserializer = [[MHVResponseDeserializer alloc] init];
        _sanitizer = [[MHVSanitizer alloc] init];

        _downloadTaskResponseTypes = @[@"NSURL*", @"NSURL"];

        AFHTTPRequestSerializer* afhttpRequestSerializer = [AFHTTPRequestSerializer serializer];
        MHVJSONRequestSerializer * swgjsonRequestSerializer = [MHVJSONRequestSerializer serializer];
        _requestSerializerForContentType = @{kMHVApplicationJSONType : swgjsonRequestSerializer,
...
- (NSURLSessionDataTask*) taskWithCompletionBlock: (NSURLRequest *)request completionBlock: (void (^)(id, NSError *))completionBlock {
    ...
    NSURLSessionDataTask *task = [self dataTaskWithRequest:request completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
        MHVDebugLogResponse(response, responseObject,request,error);
        if(!error) {
        ...
        if (responseObject) {
            ...
            userInfo[MHVResponseObjectErrorKey] = responseObject;
        }
...
- (NSURLSessionDataTask*) downloadTaskWithCompletionBlock: (NSURLRequest *)request completionBlock: (void (^)(id, NSError *))completionBlock {
    ...
    NSURLSessionDataTask* task = [self dataTaskWithRequest:request completionHandler:^(NSURLResponse *response, id responseObject, NSError *error) {
        ...
        NSString *filename = MHV__fileNameForResponse(response);
...
- (NSURLSessionTask*) requestWithPath: (NSString*) path
    ...
    __weak id<MHVSanitizer> sanitizer = self.sanitizer;
    ...
    [pathParams enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        ...
        safeString = MHVPercentEscapedStringFromString(safeString);
...
- (NSString*) pathWithQueryParamsToString:(NSString*) path queryParams:(NSDictionary*) queryParams {
    ...
    for(NSString * key in [queryParams keyEnumerator]){
        ...
        NSString *safeKey = MHVPercentEscapedStringFromString(key);
        if ([queryParam isKindOfClass:[NSString class]]){
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, MHVPercentEscapedStringFromString(queryParam)]];

        } else if ([queryParam isKindOfClass:[MHVQueryParamCollection class]]){
            MHVQueryParamCollection * coll = (MHVQueryParamCollection*) queryParam;
            ...
            if([format isEqualToString:@"multi"]) {
                for(id obj in values) {
                    ...
                    NSString * safeValue = MHVPercentEscapedStringFromString([NSString stringWithFormat:@"%@",obj]);
            ...
            NSString * safeValue = MHVPercentEscapedStringFromString([values componentsJoinedByString:separatorStyle]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        } else {
            NSString * safeValue = MHVPercentEscapedStringFromString([NSString stringWithFormat:@"%@",queryParam]);
            [requestUrl appendString:[NSString stringWithFormat:@"%@%@=%@", separator, safeKey, safeValue]];
        }
        counter += 1;
    }
...
- (void) updateHeaderParams:(NSDictionary * *)headers queryParams:(NSDictionary * *)querys WithAuthSettings:(NSArray *)authSettings {
    ...
    id<MHVConfiguration> config = self.configuration;
