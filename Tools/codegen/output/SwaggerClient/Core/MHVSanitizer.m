...
NSString * const kMHVApplicationJSONType = @"application/json";

NSString * MHVPercentEscapedStringFromString(NSString *string) {
    static NSString * const kMHVCharactersGeneralDelimitersToEncode = @":#[]@";
    static NSString * const kMHVCharactersSubDelimitersToEncode = @"!$&'()*+,;=";

    NSMutableCharacterSet * allowedCharacterSet = [[NSCharacterSet URLQueryAllowedCharacterSet] mutableCopy];
    [allowedCharacterSet removeCharactersInString:[kMHVCharactersGeneralDelimitersToEncode stringByAppendingString:kMHVCharactersSubDelimitersToEncode]];
...
- (id) sanitizeForSerialization:(id) object {
    ...
    else if ([object isKindOfClass:[NSString class]] || [object isKindOfClass:[NSNumber class]] || [object isKindOfClass:[MHVQueryParamCollection class]]) {
        return object;
    }
    ...
    else if ([object isKindOfClass:[MHVObject class]]) {
...
- (NSString *) selectHeaderAccept:(NSArray *)accepts {
    ...
    for (NSString *string in accepts) {
        if ([self.jsonHeaderTypeExpression matchesInString:string options:0 range:NSMakeRange(0, [string length])].count > 0) {
            return kMHVApplicationJSONType;
        }
